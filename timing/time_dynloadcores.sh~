...
OUTPUTNAME=dynload_new_100µs_wp$WP\_info.csv
...
# ------------------------------------------------------- #
echo -n "" > $OUTPUT
 echo "Cores;Input.Size;Time;Loops.MALMS;Time.MCSTL;Loops.MCSTL;Workpakets" >> $OUTPUT
	...
	do
		...
		do
			...
			do
				...
				if [ "$algo" = "mcstl" ]; then
					./dynloadcores ./timesortfile -k $cores -a $algo input.data >> $OUTPUT &
				else
					./dynloadcores ./timesortfile -k $WP -c $malmscores -a $algo input.data >> $OUTPUT &
				fi
				PID_OF_SORT=$!
